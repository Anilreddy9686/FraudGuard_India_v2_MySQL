...
@admin_required
def all_users():
    # Fetch all users with their transaction statistics
    users = query("""
        SELECT u.*, 
               COUNT(t.id) AS txn_count, 
               SUM(CASE WHEN t.prediction = 'Fraud' THEN 1 ELSE 0 END) AS fraud_count, 
               COALESCE(SUM(t.amount_inr), 0) AS total_volume 
        FROM users u 
    ...
    # Fetch general system user statistics
    stats = query_one("""
...
def profile():
    ...
    if request.method == "POST":
        ...
        if action == "update_profile":
            execute("""
                UPDATE users SET full_name = %s, mobile = %s, state = %s, bio = %s 
                WHERE id = %s
            ...
            cur_status = user.get("otp_enabled", 0)
            execute("UPDATE users SET otp_enabled = %s WHERE id = %s", (0 if cur_status else 1, uid))
            flash(f"OTP {'disabled' if cur_status else enabled}.", "success")
