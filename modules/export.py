...
def export_csv():
    ...
    if is_admin:
        txns = query("SELECT t.*, u.username FROM transactions t JOIN users u ON t.user_id=u.id ORDER BY t.created_at DESC")
    else:
        txns = query("SELECT * FROM transactions WHERE user_id=%s ORDER BY created_at DESC", (uid,))
...
def export_report():
    ...
    if is_admin:
        txns = query("SELECT t.*, u.username FROM transactions t JOIN users u ON t.user_id=u.id ORDER BY t.created_at DESC LIMIT 100")
        stats = query_one("SELECT COUNT(*) AS total, SUM(CASE WHEN prediction='Fraud' THEN 1 ELSE 0 END) AS frauds, COALESCE(SUM(amount_inr),0) AS volume FROM transactions")
    else:
        txns = query("SELECT * FROM transactions WHERE user_id=%s ORDER BY created_at DESC LIMIT 100", (uid,))
        stats = query_one("SELECT COUNT(*) AS total, SUM(CASE WHEN prediction='Fraud' THEN 1 ELSE 0 END) AS frauds, COALESCE(SUM(amount_inr),0) AS volume FROM transactions WHERE user_id=%s", (uid,))
...
<div class="meta">Generated: {datetime.now().strftime('%d %B %Y, %H:%M IST')} | User: <strong>{session.get('username')}</strong> | Role: <strong>{session.get('role','user').title()}</strong></div>
<div class="stats">
