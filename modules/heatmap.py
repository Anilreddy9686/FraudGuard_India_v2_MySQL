...
def heatmap_data():
    ...
    # Fetch rows based on role
    if is_admin:
        rows = query("SELECT id,amount_inr,prediction,risk_score,type FROM transactions ORDER BY created_at DESC LIMIT 5000")
    else:
        rows = query("SELECT id,amount_inr,prediction,risk_score,type FROM transactions WHERE user_id=%s ORDER BY created_at DESC LIMIT 1000",(uid,))
    ...
    # Summary query using CASE for cross-version compatibility
    summary_sql = "SELECT COUNT(*) AS total, SUM(CASE WHEN prediction='Fraud' THEN 1 ELSE 0 END) AS frauds, COALESCE(SUM(amount_inr),0) AS volume FROM transactions"
    if is_admin:
        summary = query_one(summary_sql)
    else:
        summary = query_one(summary_sql + " WHERE user_id=%s", (uid,))
