...
def _get_txn(txn_id, uid, is_admin):
    # Fetch transaction details along with user info for the receipt
    if is_admin:
        return query_one("""
            SELECT t.*, u.username, u.full_name, u.email, u.mobile 
            FROM transactions t 
...
def receipt_json(txn_id):
    ...
    # Convert row data to a JSON-serializable dictionary
    d = dict(txn)
    for k, v in d.items():
        if hasattr(v, "isoformat"): 
            d[k] = str(v) # Handle datetime objects
        elif hasattr(v, "__float__"): 
            d[k] = float(v) # Handle Decimal objects
